...
- Viewer-mode pairing toggle in the web Connect Panel before a host session is established.
- One-tap fallback action that retries pairing as a viewer when controller access is blocked by an active lease.
- Browser regression coverage for controller and viewer pairing flows in the updated Connect Panel.

## [1.14.0] - 2026-03-20

### Fixed

- Corrected the shared phone-shell hero widths so the home dashboard no longer collapses title and description text into unusably narrow columns.
- Improved web pairing conflict feedback when another controller already holds the active host lease.
- Added regression coverage for the phone-shell layout and controller-lease conflict copy discovered during dogfooding.
...
- Shared web host transport that reuses one websocket per host session.
- Thread and review subscription fan-out across timeline, command-console, and review-list views.
- Browser coverage for shared websocket reuse across live thread views.

## [1.11.0] - 2026-03-19

### Added

- Websocket review-list subscription and snapshot messages in the remote protocol.
- Host gateway support for review snapshot subscriptions and broadcasts.
- Live `/reviews` client panel that updates review counts and items without a page refresh.

## [1.10.0] - 2026-03-19

### Added

- State and workspace filters on the `/reviews` route.
- Filter-aware review history rendering with empty-state handling.
- Browser coverage for review state filtering.

## [1.9.0] - 2026-03-19

### Added

- Dedicated `/reviews` route for cross-workspace review history and queue inspection.
- Review-history screen with queue-state stats and deep links back into the relevant thread.
- Browser coverage for the review history route.

## [1.8.0] - 2026-03-19

### Added

- Home-screen review queue for review-ready threads and approval gates across workspaces.
- Live host mapping that projects review queue items from workspace thread summaries.
- Browser coverage for review queue rendering and thread deep links from the dashboard.
